...
renderProfile(res){
    console.log(res);
    const created_at = (new Date(res.created_at).toLocaleDateString()) // zamani gun/ay/yil olarak verir
...
showAlert(message , className){
   ...
   console.log(div)
    ...
    elements.warning.appendChild(div) //appendChild ile olusturdugumuz divi ekledik.
    console.log(elements.warning)

    //uyariyi ekrandan silme fonksiyonunu, showAlert fonksiyonunu calistiktan 3sn sonra calistirir.
    setTimeout(()=>{
        this.clearAlert()
    },3000)
...
clearAlert() {
 ...
 darkMode(){
    ...
    elements.title.classList.toggle("text-dark") // toggle : tiklandiiginda ozellik classlist'te yoksa ekliyor,varsa cikartiyor.
    
 }

 renderProjects(data){  // forEach veya map ıle 2 sekılde yapabılırız.
    //* projeler dizisindeki her bir eleman icin kart olusturur ve ekrana basar.forEach:elemani döndükten sonra bırakıyor.
    data.forEach((repo)=> {
        this.repoArea.innerHTML +=`
   ...
   <div class="col-6">
     <span class="badge bg-secondary">Star :${repo.stargazers_count}</span>
     <span class="badge bg-primary">Fork :${repo.forks_count}</span>
     <span class="badge bg-success">Watching :${repo.watchers}</span>
   </div>
 </div>`
    })


//* projeler dizisindeki her bir eleman icin kart olusturur ve ekrana basar. map :her döndüğü eleman için bir dizi olusturuyor.
    // data.map((repo)=>{
    //     this.repoArea.innerHTML +=`
    //     <div class="border row p-3 mb-3">
    //     <div class="col-6">
